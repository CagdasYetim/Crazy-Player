...
var playerTranslationX = 0,
    playerTranslationY = 0.3,
    playerTranslationZ = -0.5;
var playerScaleX = 0.05,
    playerScaleY = 0.05,
    playerScaleZ = 0.02;
var playerScaleArmX = playerScaleX / 4,
    playerScaleArmY = (playerScaleY * 3) / 4,
    playerScaleArmZ = playerScaleZ / 2,
    playerScaleLegX = playerScaleArmX,
    playerScaleLegY = playerScaleArmY,
    playerScaleLegZ = playerScaleArmZ;

var playerTranslationRightArmX = -0.065,
    playerTranslationRightArmY = 0,
    playerTranslationRightArmZ = 0,
    playerTranslationLeftArmX = 0.065,
    playerTranslationLeftArmY = 0,
    playerTranslationLeftArmZ = 0;
var playerTranslationRightLegX = -0.03,
    playerTranslationRightLegY = -0.09,
    playerTranslationRightLegZ = 0,
    playerTranslationLeftLegX = 0.03,
    playerTranslationLeftLegY = -0.09,
    playerTranslationLeftLegZ = 0;


var playerScaleHeadX = playerScaleX / 3,
    playerScaleHeadY = playerScaleY / 3,
    playerScaleHeadZ = playerScaleZ;
var playerTranslationHeadX = 0,
    playerTranslationHeadY = 0.0675,
    playerTranslationHeadZ = 0;

//------------ball on Platform Variables--------------
var ballTranslationX = -0.4,
    ballTranslationY = 0.04,
    ballTranslationZ = -0.5;
var ballScaleX = 0.007,
    ballScaleY = 0.007,
    ballScaleZ = 0.007;
var ballTransformationNode;
var ballSpinning = false;
var ballRotationSpeed = 30;
var ballFlying = 0;
...
var platformRotationAngle = 1,
    platformRotationAngleSpeed = 30;
var platformTranslationX = 0,
    platformTranslationY = 0,
    platformTranslationZ = -0.1;
var platformScaleX = 0.4,
    platformScaleY = 0.4,
    platformScaleZ = 0.1;
//------------------------------------------
...
var cameraTranslationX = 0,
    cameraTranslationY = -1.85,
    cameraTranslationZ = -2.97,
    cameraTranslationSpeed = 1.2;
var cameraRotationAngle = -90,
    cameraRotationAngleSpeed = 40;
//------------------------------------------
...
function initBall(scene) {
    var ballTransformationMatrix = mat4.create();
    ballTransformationMatrix = mat4.multiply(mat4.create(), ballTransformationMatrix, glm.translate(ballTranslationX, ballTranslationY, ballTranslationZ));
    ballTransformationMatrix = mat4.multiply(mat4.create(), ballTransformationMatrix, glm.scale(ballScaleX, ballScaleY, ballScaleZ));
    ballTransformationNode = new TransformationSceneGraphNode(ballTransformationMatrix);
    scene.append(ballTransformationNode);
    ballTransformationNode.append(new BallRenderNode());
}
...
var playerRotationArmAngle = 0,
    playerRotationArmAngleSpeed = 25,
    leftArmIsBack = true;
var playerRotationAngle = -30,
    playerRotationAngleSpeed = 50,
    playerTranslationSpeed = 0.25;
...
function updatePlayerOnPlayGround() {
    ...
    if (!isNaN(deltaTime) && !isNaN(time)) {
        ...
        if (time > 10 && time < 19.5) {
        ...
        } else if (time > 19.5 && time < 30) { //TODO
            ...
            if (time >= 19.5 && time < 20.5) {
                ...
                playerTranslationX = playerTranslationX + (deltaTime * -0.5);
                //console.log(playerTranslationX);
                if (playerTranslationX <= -0.3) {
                    ballSpinning = true;
                    ballTranslationX = ballTranslationX + (deltaTime * -1);
                    ballFlying = deltaTime * 0.3;
                    ballTranslationY = ballTranslationY + ballFlying;
                }
                updateArmAngle();
                updateArms();
                updateLegs();
            } else if (time >= 21.5 && time <= 21.9) {
                ballTranslationX = ballTranslationX + (deltaTime * -1);
                ballFlying = deltaTime * 0.3;
                ballTranslationY = ballTranslationY + ballFlying;
                // TODO get arc with sine
                //ballFlying += Math.PI / 64;
                //ballFlying %= 2 * Math.PI;
                //ballTranslationY = ballTranslationY + Math.sin(ballFlying) * 0.05;
            } else if (time >= 21.9 && time <= 30) {
                if(ballTranslationX >= -1.95) {
                    ballTranslationX = ballTranslationX + (deltaTime * -1);
                }
                if(ballTranslationY > 0.04) {
                    ballFlying = deltaTime * -0.3;
                    ballTranslationY = ballTranslationY + ballFlying;
                    if(ballTranslationY < 0.04) {
                        ballTranslationY = 0.04;
    ...
    ballTransformationMatrix = mat4.multiply(mat4.create(), ballTransformationMatrix, glm.scale(ballScaleX, ballScaleY, ballScaleZ));
    if (ballSpinning) {
        ballTransformationMatrix = mat4.multiply(mat4.create(), ballTransformationMatrix, glm.rotateX(ballRotationSpeed));
    }
    ballTransformationNode.setMatrix(ballTransformationMatrix);
...
class BallRenderNode extends SceneGraphNode {
    render(contex) {
        renderBall(context);
        super.render(context);
    }
